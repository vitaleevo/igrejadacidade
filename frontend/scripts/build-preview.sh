...
# As rotas dinâmicas (src/app/api, src/app/[locale]/(admin)) são temporariamente
...
restore() {
  ...
  [ -d "$STASH_DIR/(admin)" ] && mv "$STASH_DIR/(admin)" "src/app/[locale]/(admin)"
  rmdir "$STASH_DIR" 2>/dev/null || true
}
trap restore EXIT

mv src/app/api "$STASH_DIR/api"
mv "src/app/[locale]/(admin)" "$STASH_DIR/(admin)"
